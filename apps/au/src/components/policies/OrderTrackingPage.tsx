...
function TrackingFaqItem({
  ...
  return (
    ...
    >
      ...
      >
        <span className="juujo-display text-base font-normal text-[var(--plum)] sm:text-lg">{question}</span>
        <span
...
export function OrderTrackingPage() {
  ...
  async function handleTrackSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError("We could not reach the Juujo order system right now. Please try again in a moment.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[var(--cream)] py-16 md:py-24">
      <div className="juujo-glow -left-20 top-12 h-[340px] w-[340px] bg-[#f4a17b] opacity-10" />
      <div className="juujo-glow -right-20 bottom-12 h-[380px] w-[380px] bg-[#a05080] opacity-10" />

      <div className="juujo-wrap relative z-10 max-w-4xl px-4 sm:px-6">
        <div className="mb-12 text-center">
          <SectionHeading
            eyebrow="Delivery Help"
            title={
              <>
                Track Your <em className="juujo-italic">Order</em>
              </>
            }
            ...
            {trackingResult && (
              <div className="mt-8 space-y-6 border-t border-[var(--border)] pt-8">
                ...
                <div className="space-y-4">
                  <h4 className="juujo-display flex items-center gap-2 text-lg text-[var(--plum)]">
                    <Truck size={18} className="text-[var(--gold)]" />
                    {trackingResult.trackingEvents.length ? "Shipment Timeline" : "Official Juujo Status"}
                  </h4>
                  ...
                  {trackingResult.trackingEvents.length ? (
                    ...
                    <p className="rounded-2xl border border-[var(--border)] bg-[var(--cream)] p-4 text-sm font-light leading-6 text-[var(--muted)]">
                      No carrier scan history is available yet. This page will show the real carrier timeline once Juujo receives it from the fulfillment system.
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="mx-auto max-w-2xl space-y-8">
          <h3 className="juujo-display mb-6 text-center text-2xl text-[var(--plum)]">Tracking FAQ</h3>
