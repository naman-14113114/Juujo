...
function errorMessage(code: string) {
  if (code === "email_not_matched") {
    return "That email does not match the order number. Use the exact email from the Juujo checkout confirmation.";
  }
  if (code === "order_not_found" || code === "not_found") {
    return "No matching Juujo order was found. Check the order number from your confirmation email and try again.";
  }
  if (code === "email_invalid_format") return "Enter a valid email address.";
  return "We could not verify that order right now. Please try again or contact support@juujo.com.";
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    lookup = await fetch(`${plusbaseStoreUrl}/api/checkout/order-status/url.json`, {
      ...
      headers: {
        ...
        "user-agent": "Mozilla/5.0 JuujoOrderTracking/1.0",
    ...
    return NextResponse.json(
      { status: "error", message: "We could not reach Juujo tracking right now. Please try again." },
      { status: 502 },
    );
  }
  ...
  if (!token || !accessKey) {
    return NextResponse.json(
      { status: "error", message: "Juujo found the order, but did not return a secure status token." },
      { status: 502 },
    );
  }
  ...
  return NextResponse.json({
    ...
    message: trackingAvailable
      ? "Real tracking details found from Juujo checkout."
