...
export function AuthForm({ mode, redirectTo }: AuthFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <label className="block">
        <span className="juujo-mono text-[var(--plum)]">Password</span>
        <span className="relative block">
          <LockKeyhole
            className="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 text-[var(--gold)]"
            size={18}
      ...
      <p className="text-center text-sm text-[var(--muted)]">
        {isSignIn ? "New to Juujo?" : "Already have an account?"}{" "}
        <Link
          className="font-semibold text-[var(--plum)] underline underline-offset-4"
          href={
            isSignIn
              ? `https://juujo.com/sign-up${redirectTo ? `?redirectTo=${encodeURIComponent(redirectTo)}` : ""}`
              : `https://juujo.com/sign-in${redirectTo ? `?redirectTo=${encodeURIComponent(redirectTo)}` : ""}`
          }
        >
