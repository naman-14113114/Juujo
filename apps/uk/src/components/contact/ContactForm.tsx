...
function buildMailto(formData: FormData) {
  const subject =
    (formData.get("subject")?.toString().trim() || "Juujo contact request");
  const body = [
...
function buildWeb3FormsData(
  ...
  web3FormsData.set("subject", `[Juujo Contact] ${subject}`);
  web3FormsData.set("from_name", `${firstName} ${lastName}`.trim() || "Juujo customer");
...
function Field({
  ...
  return (
    <label className={cn("block", className)}>
      <span className="juujo-mono text-[var(--plum)]">
        {label}
        {required ? " *" : ""}
      </span>
      <input
  ...
  return (
    ...
    >
      ...
      <label className="mt-4 block">
        <span className="juujo-mono text-[var(--plum)]">Message *</span>
        <textarea
